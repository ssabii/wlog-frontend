...
declare namespace naver.maps {
  ...
  namespace Service {
    ...
    interface ReverseGeocodeResultV2 {
      code: {
        id: string,
        type: string,
        mappingId: string,
      ...
      name: string,
      region: {
        area0: {name: string, coords: {}}
        area1: {name: string, coords: {}}
        area2: {name: string, coords: {}}
        area3: {name: string, coords: {}}
        area4: {name: string, coords: {}}
      }
      land: {
        type: string;
        number1: string;
    ...
    enum ReverseGeocodeStatusCodeEnum {
      CODE_0 = 0, // 정상적인 기능수행 및 반환된 결과가 있는 경우
      CODE_3 = 3 ,// 정상적인 기능수행 및 반환된 결과가 없는 경우
      CODE_100 = 100, // 요청 파라미터가 잘못된 경우
      CODE_900 = 900, // 정의되지 않은 에러
    }

    enum ReverseGeocodeStatusNameEnum {
      OK = "ok", // 정상적인 기능수행 및 반환된 결과가 있는 경우
      NO_RESULTS = "" ,// 정상적인 기능수행 및 반환된 결과가 없는 경우
      INVALID_REQUEST = "invalid request", // 요청 파라미터가 잘못된 경우
      UNKNOWN_ERROR_IO_ERROR = "unknown error / io error ", // 정의되지 않은 에러
    }

    interface ReverseGeocodeResponse {
      v2: {
        results: ReverseGeocodeResultV2[];
        status: {
          code: ReverseGeocodeStatusCodeEnum;
          message: string;
          name: ReverseGeocodeStatusNameEnum;
        }
      }
      result: {
        userquery: string;
